...
def normalize_datetime_for_future(start_dt, tz_name):
    """Normalize a datetime to be in the future."""
    try:
        local = ZoneInfo(tz_name)
        now_local = dt.now(local)
        s_local = start_dt.astimezone(local)
        if s_local < now_local:
            ...
            return candidate.astimezone(start_dt.tzinfo)
        return start_dt
    except Exception:
        return start_dt